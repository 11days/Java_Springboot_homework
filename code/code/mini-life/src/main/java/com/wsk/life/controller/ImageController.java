...
public class ImageController {
    ...
    @GetMapping("/crossorigin/{proxy}")
    public void getImg(@PathVariable String urlEncode, HttpServletResponse response) throws Exception {
        //创建输出流  输出到本地
        ...
        try {
            httpsURLConnection = httpsRequest.getHttpsConnection(url);
            httpsURLConnection.connect();
            InputStream inputStream = httpsURLConnection.getInputStream();
            byte[] body = Utils.streamToByteArray(inputStream);
            os.write(body);
            os.flush();
        }
        catch (Exception ex){
