...
//        model.addAttribute("token", token);
    ...
    @RequestMapping(value = "checkRegisterPhone", method = {RequestMethod.POST, RequestMethod.GET})
    @ResponseBody
    public Map checkRegisterPhone(@RequestParam String phone, @RequestParam String photo, @RequestParam String code_phone,HttpServletRequest request) {
        ...
        UserInformation userInfo = userInformationService.selectByPhone(phone);
        if(userInfo != null){
            map.put("result", "4");//手机号码已注册
            return map;
        }
    ...
    @ResponseBody
    public Map checkPhone(@RequestParam String phone, @RequestParam String photo, @RequestParam String code_phone,HttpServletRequest request) {
        Map<String, String> map = new HashMap<>();
        map.put("result", "1");//true result;
        if (!checkPhoto(photo, request)) {
            map.put("result", "2");//图片验证码错误
...
//        }
    ...
    private boolean checkPhoto(String photo, HttpServletRequest request) {
        ...
        String true_photo =  session.getAttribute("rand") != null ? session.getAttribute("rand").toString():"";
        return StringUtils.isEmpty(photo) == false && true_photo.equals(photo);
    }

    //check the phone`s code
    private boolean checkCodePhone(String code_phone, HttpServletRequest request) {
        HttpSession session = request.getSession();
        String true_code_phone = (String)session.getAttribute("codePhone");
