...
public class WangYiServiceImpl implements WangYiService {
    ...
    @Autowired
    public WangYiServiceImpl(IRedisUtils redisUtils, WangYiMusicRepository musicService,
                             WangYiSingerRepository singerService, WangYiAlbumRepository albumService, WangYiLrcRepository lrcRepository) {
    ...
    public BaseEntity getMusic(String name, String type) {
        result.clear();
        //从redis查找
        String redisKey = RedisKey.getMusicKey("wangyi:songName:" + name);
        List<String> list = redisUtils.lrange(redisKey, 0, -1);
        ...
        if (null != wangyimusicEntities && wangyimusicEntities.size() > 5) {
            ...
            for (WangyimusicEntity music : wangyimusicEntities) {
                ...
                result.add(entity);
                String redisNameKey = RedisKey.getMusicKey("wangyi:songName:" + name);
                redisUtils.rpush(redisNameKey, JsonUtil.toJson(entity));
            }
            responseEntity.setMsg("success");
            responseEntity.setCode(200);
            responseEntity.setData(result);
    ...
    public BaseEntity getMusicComments(long song_id) {
        ...
        entity.setCode(500);
        entity.setMsg("fail");
        String redisKey = RedisKey.getMusicKey(COMMENT_CACHE + song_id);
        String result = redisUtils.hget(redisKey , String.valueOf(song_id));
    ...
    public boolean delRedisUrl(String url) {
        String redisKey = RedisKey.getMusicKey(url);
        return redisUtils.del(redisKey);
    }
...
//        System.out.println("new=======");
    ...
    public BaseEntity getMusicUrlById(long song_id) {
        ...
        String redisKey = RedisKey.getMusicKey("wangyi:url:" + song_id);
        String url = redisUtils.get(redisKey);
        ...
        if (Tool.getInstance().isNotNull(json)) {
            ...
            for (WangYiSong song : wangYiResult.getSongs()) {
                try {
                    ...
                    String redisIdKey = RedisKey.getMusicKey("wangyi:url:" + entity.getSongid());
                    String song_url = redisUtils.get(redisIdKey);
...
//                    e.printStackTrace();
        ...
        try {
            ...
            String redisKeyUrl = RedisKey.getMusicKey(url);
            redisUtils.set(redisKeyUrl, json, Time.FIVE_MINUTE);
...
//                如果该歌曲数据库或者缓存已经存在了，则直接获取数据库的歌曲
                String redisKey = RedisKey.getMusicKey("wangyi:url:"+song_id);
                String song_url = redisUtils.get(redisKey);
                WangYiEntity entity = new WangYiEntity();
                if (Tool.getInstance().isNullOrEmpty(song_url)) {
...
//                    singers.add(singer);
                ...
                if (Tool.getInstance().isNullOrEmpty(searchName)) {
                    redisKeyName = RedisKey.getMusicKey("wangyi:songName:" + song.getName());
                    redisUtils.rpush(redisKeyName, JsonUtil.toJson(entity));
                } else {
                    redisKeyName = RedisKey.getMusicKey("wangyi:songName:" + searchName);
                    redisUtils.rpush(redisKeyName, JsonUtil.toJson(entity));
                }
        ...
        try {
            ...
            if (object.containsKey("lrc")){
                ...
                if (lrc.containsKey("lyric")) {
                    ...
                    //保存缓存
                    String redisKey = RedisKey.getMusicKey(LYRIC_CACHE);
                    redisUtils.hset(redisKey, String.valueOf(song_id), lyric, Time.ONE_DAY);
                    return lyric;
                }
            }
            return null;
