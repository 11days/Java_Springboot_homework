...
public class UserInformationController extends AsideController {
    ...
    public String cc(HttpServletRequest request, Model model) {
       ...
       //his is a home when user login for my web
    @RequestMapping(value = "home", method = RequestMethod.GET)
    public String home(HttpServletRequest request, Model model) {
        UserInformation userInfo = JwtUtil.getLoginUser();
        if (userInfo == null) {
...
//            name = Tool.getInstance().txtReplace(name);
    ...
    public String home(@RequestParam String username, @RequestParam String password,
        ...
        try {
            int id = getUserInformationId(username);
            if (id == -1){
                throw new Exception("账号或密码错误");
            };
            String psw = userPasswordService.getPassword(id).getPassword();
            String passwordMd5 = Tool.getInstance().getMD5(password);
            if (passwordMd5.equals(psw)) {
                ...
                map.put("userName", userInformation.getName());
                map.put("phone", userInformation.getPhone());
                //生成token
                String token = JwtUtil.generateJwt(map);
                request.getSession().setAttribute("token", token);
                CookieUtil.setCookie(response,"token",token,86400);
                setUserInfoToRedis(token,userInformation);
                ...
                if (!Tool.getInstance().isNullOrEmpty(remember_me)) {
                    CookieUtil.setCookie(response,"MovieWeb_username",username,86400);
                    CookieUtil.setCookie(response,"MovieWeb_password",password,86400);
                    CookieUtil.setCookie(response,"remember_me","on",86400);
                }
                else {
                    CookieUtil.clearCookie(response,"MovieWeb_username");
                    CookieUtil.clearCookie(response,"MovieWeb_password");
                    CookieUtil.clearCookie(response,"remember_me");
                }
                return "redirect:/home";
            }
            else{
